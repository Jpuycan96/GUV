...
export class ServicioService {
  ...
  // crea un nuevo servicio (sin imagen)
  crear(dto: ServicioDTO): Observable<ServicioDTO> {
    return this.http.post<ServicioDTO>(this.apiUrl, dto);
  }

  // ✨ crear con imagen
  crearConImagen(formData: FormData): Observable<ServicioDTO> {
    return this.http.post<ServicioDTO>(this.apiUrl, formData);
  }

  // actualiza un servicio (sin imagen)
  actualizar(id: number, dto: ServicioDTO): Observable<ServicioDTO> {
    return this.http.put<ServicioDTO>(`${this.apiUrl}/${id}`, dto);
  }

  // ✨ actualizar con imagen
  actualizarConImagen(id: number, formData: FormData): Observable<ServicioDTO> {
    return this.http.put<ServicioDTO>(`${this.apiUrl}/${id}`, formData);
  }
